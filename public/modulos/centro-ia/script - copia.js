...
function updateStats() {
  ...
  const vision = models.find(m => m.activo && Array.isArray(m.capacidades) && m.capacidades.includes('vision'));
...
function buildModelCard(m) {
  ...
  const maskedKey = m.apiKey
    ? m.apiKey.slice(0, 8) + '•'.repeat(Math.min(16, Math.max(0, m.apiKey.length - 8)))
    : '(no configurada)';
  ...
  return `
    <div class="model-card" data-model-id="${m.id}">
      <div class="model-card__head">
        ...
        <span class="badge badge--${m.activo ? 'active' : 'inactive'}">
          <span class="badge__dot"></span>${m.activo ? 'Activo' : 'Inactivo'}
        </span>
      </div>
      <div class="model-card__body">
        ...
        <div class="model-card__field">
          ...
          <div class="model-card__caps">${capsHtml || '<span style="color:var(--color-text-muted)">Ninguna</span>'}</div>
        </div>
      </div>
      <div class="model-card__foot">
        <div class="model-card__actions">
          <button class="btn btn--outline btn--sm" data-action="test" data-id="${m.id}"><span class="btn__icon" data-icon="check"></span>Probar</button>
          <button class="btn btn--outline btn--sm" data-action="edit" data-id="${m.id}"><span class="btn__icon" data-icon="edit"></span>Editar</button>
          <button class="btn btn--danger  btn--sm" data-action="del"  data-id="${m.id}"><span class="btn__icon" data-icon="trash"></span>Eliminar</button>
        </div>
...
function syncProviderFields(proveedor) {
  ...
  const ep            = PROVIDERS[proveedor]?.endpoint || '';
...
function bindEvents() {
  ...
  document.getElementById('model-form').addEventListener('submit', e => {
    ...
    if (editingId) { updateModel(editingId, data); } else { addModel(data); }
    showSection('section-bandeja');
    renderModels();
  });
